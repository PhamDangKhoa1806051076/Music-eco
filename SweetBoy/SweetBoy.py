...
class LyricCard:
    ...
    def __init__(self, master, text, x, y, speed_ms=70, color_theme=None, on_close=None):
        ...
        for widget in (self.win, self.inner_frame, self.label):
            ...
            # Nhấp đúp chuột phải hoặc chuột trái để đóng thẻ note sớm nếu muốn
...
class LyricFloatApp:
    ...
    """
    def __init__(self, audio_file="SweetBoy.mp3", lyrics_file="lyrics.txt"):
        self.root = tk.Tk()
    ...
    def start(self):
        ...
        print("=" * 65)
        print("  🎵 FLOATING LYRICS PLAYER - MALCOLM TODD (SWEET BOY)")
...
def find_audio_file():
    """Tự động ưu tiên tìm file SweetBoy.mp3 trong thư mục hiện tại."""
    dir_path = os.path.dirname(os.path.abspath(__file__))
    candidates = [
        os.path.join(dir_path, "SweetBoy.mp3"),
        os.path.join(dir_path, "endoftheroad.mp3"),
        os.path.join(dir_path, "demo_music.wav")
    ]
    for c in candidates:
        if os.path.exists(c):
            return c
    ...
    return os.path.join(dir_path, "demo_music.wav")
