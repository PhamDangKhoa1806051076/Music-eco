...
class LyricFloatApp:
    ...
    """
    def __init__(self, audio_file="Starboy.mp3", lyrics_file="lyrics.txt"):
        self.root = tk.Tk()
    ...
    def start(self):
        ...
        print("=" * 65)
        print("  🎵 FLOATING LYRICS PLAYER - THE WEEKND (STARBOY)")
...
def find_audio_file():
    """Tự động ưu tiên tìm file Starboy.mp3 trong thư mục hiện tại."""
    dir_path = os.path.dirname(os.path.abspath(__file__))
    candidates = [
        os.path.join(dir_path, "Starboy.mp3"),
    ]
    for c in candidates:
        if os.path.exists(c):
            return c
    ...
    return os.path.join(dir_path, "Starboy.mp3")
